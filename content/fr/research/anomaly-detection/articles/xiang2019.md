---
publish: false
title: LAMOST DR5 — Abundâncias de 16 Elementos
created: 2026-07-18
modified: 2026-07-25T23:58:08.057-03:00
published: 2026-07-25T23:58:08.057-03:00
tags:
  - artigo
  - pesquisa
---

> [!note] En résumé
> Déterminer les paramètres et l'abondance de 6 millions d'étoiles de LAMOST DR5 via le modèle DD-Payne (herda The Payne et The Cannon), avec 16 éléments.

 Xiang, Maosheng. (2019)

## Synthèse

 Déterminer les paramètres et l'abondance de 6 millions d'étoiles de LAMOST DR5 via le modèle DD-Payne (herda The Payne et The Cannon), avec 16 éléments. Il montre la puissance des modèles fondés sur les données dans les grands soulèvements - pertinent à son pipeline d'abondance.

 [Voir article original](https://doi.org/10.3847/1538-4365/ab5364)·[Réflexion Télécharger le PDF](assets/articles/Xiang2019.pdf)

## Citation

 bibtex
 @ ARTICLE {XIang2019,
 auteur = {{Xiang}, Maosheng et {Ting}, Yuan-Sen et {Rix}, Hans-Walter et {Sandford}, Nathan et {Buder}, Sven et {Lind}, Karin et {Liu}, Xiao-Wei et {Shi}, Jian-Rong et {Zhang}, Hua-Wei},
 titre = « {Abondance des estimations pour 16 éléments dans 6 millions d'étoiles de LAMOST DR5 à faible résolution Spectra} »,
 journal = {\ apjs},
 mots-clés = {Spectroscopie, Etoiles binaires spectroscopiques, Atmospheres Stellar, Abondances Stellar, Bases de données d'astronomie, Propriétés stellar, Lignes spectrales Stellar, Analyse des données d'astronomie, Paramètres fondamentaux des étoiles, La Voie lactée Galaxy, Modélisation des données d'astronomie, 1558, 1557, 1584, 1577, 83, 1624, 1630, 1858, 1464, 555, 1054, Astrophysique et astrophysique - Astrophysique, Astrophysique et astrophysique - Astrophysique, Astrophysique et solaire,
 année = 2019,
 mois = diminution,
 volume = {245},
 nombre = {2},
 eid = {34},
 pages = {34},
 doi = {10,3847 / 1538-4365 / ab5364},
 archivePrefix = {arXiv},
 eprint = {1908.09727},
 classe primaire = {astro-ph.SR},
 adsurl = {https: / / ui.adsabs.harvard.edu / abs / 2019ApJS.. 245... 34X},
 adsnote = {Fourni par le système de données astrophysiques SAO / NASA}
 }
 ""

> [!abstract] Avis de traduction automatique
> Cette page a été traduite automatiquement du portugais à l'aide du traducteur automatique basé sur LibreTranslate implémenté dans `tools/translate_quartz.py` (qui préserve les wikilinks, les embeds et les noms propres par découpage positionnel). Il s'agit d'une traduction automatique pouvant contenir des inexactitudes — la version portugaise originale fait foi.
