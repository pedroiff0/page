...
aliases:
  - Short Lecture FCT
  - Resumo Geral FCT
  - Filosofia da Ciência e Tecnologia - Síntese Teórica
created: 2026-08-22
---
...
```mermaid
flowchart TD
    A[Teoria do Conhecimento: Epistemologia] --> B[Diferenciação: Téchne, Episteme, Ciência, Engenharia]
    B --> C[O Paradigma Científico Moderno & Mecanicismo]
    C --> D[Debate sobre o Progresso Científico]
    D --> D1[Popper: Falseacionismo & Demarcação]
    D --> D2[Kuhn: Paradigmas & Revoluções Científicas]
    D --> D3[Lakatos & Feyerabend: Pluralismo & Métodos]
    D --> E[Crise do Paradigma Dominante: Boaventura & Morin]
    E --> F[Estudos CTS & Sociologia da Ciência: Bruno Latour & ANT]
    F --> G[Teoria Crítica da Tecnologia: Heidegger, Marcuse, Álvaro Vieira Pinto]
    G --> H[Ética da Tecnologia & Século XXI: Hans Jonas & IA]
```
...
```mermaid
flowchart LR
    subgraph Popper [Karl Popper]
        P1[Falseacionismo] --> P2[Critério de Demarcação]
        P2 --> P3[Conjecturas & Refutações]
    end
    subgraph Kuhn [Thomas Kuhn]
        K1[Ciência Normal] --> K2[Crise & Anomalias]
        K2 --> K3[Revolução Científica]
        K3 --> K4[Novo Paradigma: Incomensurabilidade]
    end
    subgraph Feyerabend [Paul Feyerabend]
        F1[Contra o Método] --> F2[Anarquismo Epistemológico]
        F2 --> F3["'Tudo Vale' (Anything goes)"]
    end
```
